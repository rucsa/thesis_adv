...
def recommended_allocation (risk_profile):
    ...
    if (risk_profile == 'Defensive'):
        alloc['Stocks'] = 10
#        alloc.append(('Stocks', 10))
#        alloc.append(('Bonds', 90 ))
...
#    alloc = pd.DataFrame(alloc, columns=['Assets', 'Allocation'])
#    alloc = alloc.set_index('Assets')
...
def calculate_portfolio_value (portfolio_dict, security_dict):
    ...
    portfolio_value = sum(exposure_dict.values())
    
    for name, shares in exposure_dict.items():
        exposure_dict[name] = exposure_dict[name] / portfolio_value * 100
    return portfolio_value, exposure_dict
